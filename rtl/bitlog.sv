...
module bitlog
  #(
    ...
    parameter BITLOG_FILE                         = "../../rtl/bitlog.mem"
  )
  (
    input  wire                     clk,
    input  wire                     rstn,
  ...
  reg [LZC_COUNT_WIDTH-1:0]         pipe_0_lzc0;
  reg [LZC_COUNT_WIDTH-1:0]         pipe_0_lzc1;
  reg [LZC_COUNT_WIDTH-1:0]         pipe_0_lzc2;
  reg [LZC_COUNT_WIDTH-1:0]         pipe_0_lzc3;
  ...
  assign r_rom1_addra  = pipe_pixel0[0][LZC_BIT_WIDTH-2:LZC_BIT_WIDTH-(ROM_DEPTH+1)];
  assign r_rom1_addrb  = pipe_pixel1[0][LZC_BIT_WIDTH-2:LZC_BIT_WIDTH-(ROM_DEPTH+1)];
  assign r_rom2_addra  = pipe_pixel2[0][LZC_BIT_WIDTH-2:LZC_BIT_WIDTH-(ROM_DEPTH+1)];
  assign r_rom2_addrb  = pipe_pixel3[0][LZC_BIT_WIDTH-2:LZC_BIT_WIDTH-(ROM_DEPTH+1)];

  simple_dual_ported_rom #(
    .DATA_WIDTH     (ROM_WIDTH),
    .ADDR_WIDTH     (ROM_DEPTH),
    .MEM_FILE       (BITLOG_FILE)
  ) lut_rom1_inst
  (
    .clk            (clk),
    .r_ena          (r_rom_en),
  ...
  always_ff @ (posedge clk) begin
    if (pipe_0_tready == 1'b1) begin
      ...
      pipe_0_lzc0     <= pipe_lzc0[0];
      pipe_0_lzc1     <= pipe_lzc1[0];
      pipe_0_lzc2     <= pipe_lzc2[0];
      pipe_0_lzc3     <= pipe_lzc3[0];
    end
  ...
  generate
    ...
    assign pipe_1_tdata_wire[PIXEL_BIT_WIDTH-1:0] = {pipe_0_lzc0, r_rom1_dataa[ROM_WIDTH-1:(ROM_WIDTH-PIXEL_BIT_WIDTH)+LZC_COUNT_WIDTH]};

    if (PIXEL_PER_CYCLE > 1) begin
      assign pipe_1_tdata_wire[2*PIXEL_BIT_WIDTH-1:PIXEL_BIT_WIDTH] = {pipe_0_lzc1, r_rom1_datab[ROM_WIDTH-1:(ROM_WIDTH-PIXEL_BIT_WIDTH)+LZC_COUNT_WIDTH]};
    end

    if (PIXEL_PER_CYCLE > 2) begin
      assign pipe_1_tdata_wire[3*PIXEL_BIT_WIDTH-1:2*PIXEL_BIT_WIDTH] = {pipe_0_lzc2, r_rom2_dataa[ROM_WIDTH-1:(ROM_WIDTH-PIXEL_BIT_WIDTH)+LZC_COUNT_WIDTH]};
    end

    if (PIXEL_PER_CYCLE > 3) begin
      assign pipe_1_tdata_wire[4*PIXEL_BIT_WIDTH-1:3*PIXEL_BIT_WIDTH] = {pipe_0_lzc3, r_rom2_datab[ROM_WIDTH-1:(ROM_WIDTH-PIXEL_BIT_WIDTH)+LZC_COUNT_WIDTH]};
    end
