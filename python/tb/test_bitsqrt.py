...
from mini_isp.utils import bitsqrt

toplevel = "bitsqrt"
sources = [
    get_rtl_path() / "simple_dual_ported_rom.sv",
    get_rtl_path() / "bitsqrt.sv",
]
...
@cocotb.test()
@pytest.mark.sim
async def test_bitsqrt_basic(dut):
    ...
    # cfa = np.linspace(start=0, stop=16384, endpoint=False, num=128*128, dtype=np.uint32)
    ...
    ref = bitsqrt(cfa) / 2**16
    result = result / 2 ** (int(dut.PIXEL_BIT_WIDTH.value) / 2)
    ...
    np.testing.assert_allclose(
        result, ref, atol=2 ** (-int(dut.PIXEL_BIT_WIDTH.value) / 2)
    )
...
@pytest.mark.synth
def test_bitsqrt_synth(paramset: ParamSet):
    rtl = Rtl(
        toplevel,
        sources,
        dict(
            [
                ...
                ("BITSQRT_FILE", [str(get_rtl_path() / "bitsqrt.mem")]),
            ]
        ),
    )

    rtl.export_markdown_report("Bitsqrt", rtl.synthesize_all(family="xc7"))


@pytest.mark.sim
def test_bitsqrt_sim(paramset: ParamSet):
    rtl = Rtl(
        toplevel,
        sources,
        dict(
    ...
    rtl.simulate_all("tb.test_bitsqrt, ", waves=False)
